...
async function searchBooks() {
    try {
        const query = document.getElementById("searchInput").value.trim();  // Get the value from the search input field
...
document.getElementById("inProgressTab").addEventListener("click", () => {
    console.log("In Progress button clicked!"); // Debugging log
...
function displayLibraryBooks(activeSection = "all") {
    ...
    // Show messages if sections are empty
...
// Create the button for In Progress books to move to "Finished"
function createProgressButton(book) {
    const progressButton = document.createElement("button");
    progressButton.textContent = "Save to Finished"; // Show "Save to Finished"
...
// Update the book progress in the library
// Update the reading progress and save the updated state
